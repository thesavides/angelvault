...
import {
  Coins, Search, Loader2, DollarSign, Building2,
  Calendar, User, Eye, CheckCircle, XCircle, Clock,
...
export function AdminOffersPage() {
  ...
  useEffect(() => {
    trackEvent('page_view', { page: 'admin_offers' });
    loadOffers();
  }, [page, statusFilter]);
  ...
  const filteredOffers = searchQuery
    ? offers.filter(offer =>
        offer.project?.title?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        offer.investor?.first_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        offer.investor?.last_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        offer.developer?.first_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        offer.developer?.last_name?.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : offers;

  const totalValue = offers.reduce((sum, o) => sum + o.investment_amount, 0);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-600 mt-1">
            Monitor all SAFE note offers across the platform
          </p>
        </motion.div>
                      ...
                      {filteredOffers.map((offer) => {
                        ...
                        return (
                          <tr key={offer.id} className="border-b border-gray-100 hover:bg-gray-50">
                              ...
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-primary-100 flex items-center justify-center">
                                  {offer.project?.logo_url ? (
                                    <img src={offer.project.logo_url} alt="" className="w-8 h-8 object-contain rounded" />
                                  ) : (
                                    <Building2 className="w-5 h-5 text-primary-600" />
                                  )}
                                </div>
                                <span className="font-medium text-navy-900">{offer.project?.title || 'N/A'}</span>
                              </div>
                            </td>
                            <td className="py-4 px-6">
                              <div>
                                ...
                                <p className="text-sm text-gray-500">{offer.investor?.email}</p>
                              </div>
                            </td>
                            <td className="py-4 px-6">
                              <div>
                                <p className="font-medium text-navy-900">
                                  {offer.developer?.first_name} {offer.developer?.last_name}
                                </p>
                                <p className="text-sm text-gray-500">{offer.developer?.email}</p>
                              </div>
                            </td>
                            <td className="py-4 px-6 text-right">
                              <span className="font-semibold text-navy-900">
                                {formatCurrency(offer.investment_amount)}
                              </span>
                            </td>
                            ...
                            <td className="py-4 px-6 text-right">
                              <Link to={`/admin/safe-notes/${offer.id}`}>
