...
import {
  ...
  Search, ChevronRight, Loader2, DollarSign, User,
  Calendar, Download, AlertCircle, TrendingUp
} from 'lucide-react';
...
type NoteStatus = 'all' | 'sent' | 'signed_investor' | 'signed_founder' | 'executed' | 'cancelled';
...
export function DeveloperSAFENotesPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    trackEvent('page_view', { page: 'developer_safe_notes' });
    loadSAFENotes();
  }, []);
  ...
  const filterNotes = () => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(note =>
        ...
        note.investor?.first_name?.toLowerCase().includes(query) ||
        note.investor?.last_name?.toLowerCase().includes(query) ||
        note.investor?.company_name?.toLowerCase().includes(query)
      );
    }
  ...
  const pendingCount = safeNotes
    .filter(n => ['sent', 'signed_investor'].includes(n.status))
    .length;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Manage investment agreements for your projects
              </p>
            </div>
          </div>
        </motion.div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-navy-900">{formatCurrency(totalRaised)}</p>
        ...
        <div className="space-y-4">
          {filteredNotes.length > 0 ? (
            filteredNotes.map((note, index) => {
              ...
              const needsAction = note.status === 'sent' || note.status === 'signed_investor';
              ...
              return (
                ...
                >
                  <Link to={`/developer/safe-notes/${note.id}`}>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between">
                          <div className="flex items-start gap-4 flex-1">
                            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary-100 to-primary-50 flex items-center justify-center flex-shrink-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500 mb-3">
                                {note.investor?.company_name || 'Individual Investor'} • {note.project?.title}
                              </p>
...
export default DeveloperSAFENotesPage;
